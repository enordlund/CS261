               ...
               TYPE *candidate_path, int *c_path_len, TYPE sumDiff, TYPE parent_value) 
{
    ...
    /* FIX ME */
    if (node->left != NULL) {
    	int lengthInt = *c_path_len;
    	preorder(node->left, min_cost, path, path_len, candidate_path, &lengthInt, sumDiff, node->val);
    	/*return;*/
    }
    if (node->right != NULL) {
    	int lengthInt = *c_path_len;
    	preorder(node->right, min_cost, path, path_len, candidate_path, &lengthInt, sumDiff, node->val);
    	/*return;*/
    }
    if ((node->left == NULL) && (node->right == NULL)) {/* we have approached a leaf */
    	/* determining if candidate path is lower cost than min_cost */
    	if(LT(sumDiff, *min_cost)) {
    		/* update min_cost, path, and path_len */
    		*min_cost = sumDiff;
    		*path_len = *c_path_len;
    		/* filling path with candidate's values */
